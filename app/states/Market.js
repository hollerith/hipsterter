...
class TheMarket extends Phaser.State {
      ...
      handlers: {
        ".end-turn": function() {
          ...
          if (movesLeft) {
            ...
            view.render(
              "You still have moves remaining, is that ok?",
              "End the turn",
              "Nevermind"
            );
          } else {
  ...
  notice(tile, msg, offset) {
    var offset = offset || 70,
      ...
      text = this.game.add.text(coord.x - offset, coord.y, msg, {
        fill: "#ffffff",
        stroke: "#000000",
        strokeThickness: 2,
        font: "bold 24pt Work Sans"
      }),
      tween;
  ...
  combatNotice(report) {
    if (report.destroyed.defender) {
      ...
      this.notice(
        report.tiles.defender,
        `-${report.damageTaken.defender} health`,
        25
      );
    }
    ...
    if (report.destroyed.attacker) {
      ...
      this.notice(
        report.tiles.attacker,
        `-${report.damageTaken.attacker} health`,
        25
      );
    }
  }
  ...
  captureNotice(tile) {
    var msg = tile instanceof Tile.Income
      ? `+${((tile.income + 1) / this.market.totalIncome * 100).toFixed(2)}% market share`
      : "Captured influencer!";
  ...
  renderUI(tile) {
    ...
    if (t.piece) {
      t.pieceClass = t.piece.owner == market.humanPlayer
        ? "friendly"
        : "hostile";
    }
    ...
    this.view.render({
      ...
      turnsPercent: (market.totalTurns - market.turnsLeft) /
        market.totalTurns *
        100
    });
  ...
  endGame(reason) {
    var view = new Alert({
      onDismiss: () => {
        var market = this.market;
        var marketShares = _.filter(
          market.humanPlayer.tiles,
          t => t instanceof Tile.Income
        ),
          influencers = _.filter(
            market.humanPlayer.tiles,
            t => t instanceof Tile.Influencer
          );
        var results = Product.setRevenue(
          this.product,
          marketShares,
          influencers.length,
          this.player
