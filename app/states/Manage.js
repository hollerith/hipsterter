...
class Manage extends Phaser.State {
  ...
  create() {
    ...
    if (this.player.company.workers.length === 0) {
      this.player.company.hireEmployee(
        Worker.init(this.player.company.cofounder),
        0
      );
  ...
  enterTheMarket(p) {
    ...
    var competitor = _.sample(
      _.filter(this.player.competitors, c => !c.disabled)
    );
  ...
  showOffice() {
    ...
    this.office = new Office(
      ...
      function() {
        ...
        _.each(
          self.player.company.workers,
          self.office.addEmployee.bind(self.office)
        );
